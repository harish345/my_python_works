...
def load_alpl_driver_list_master(json,source,con):
    ...
    unitsQ = 'SELECT alpl_unit_code,unit_id from alpl_unit_master where org_id = '+ str(org_id)
    cursor.execute(unitsQ)
...
def main(file_name):  # confirms that the code is under main function
    ...
    try:
        ...
        src = json_obj[source]
        func =None
        if(src=="RMC_DRIVER_MASTER"):
            func = load_alpl_driver_list_master
        elif(src=="RMC_DRIVER_VEHICLEMAPPING_TRANS"):
            pass
            if len(jsonData)>=5000:
                pass
            func=load_alpl_driver_vehicle_mapping
        elif(src=="RMC_DRIVER_ONBOARD_TRANS"):
            pass
            if len(jsonData)>5000:
                pass
            func=load_alpl_driver_onboarding
        elif(src=="RMC_DRIVER_REST_LEAVES_TRANS"):
            pass
            func = load_alpl_driver_leaves_and_rest
        elif(src=="RMC_DRIVER_HEALTH_MEDI_TRANS"):
            pass
