...
def get_location_type(cursor):
    global loct_map
    cursor.execute("SELECT alpl_reference_id,alpl_location_type_id FROM alpl_location_type_master where org_id=2")
    res_lt  = cursor.fetchall()
    loct_map=dict(res_lt)

def get_locations(cursor):
    global loc_map
    cursor.execute("SELECT alpl_reference_id,alpl_location_master_id FROM alpl_location_master where org_id=2")
...
def get_units(cursor):
    cursor.execute('SELECT alpl_unit_code,unit_id FROM alpl_unit_master where org_id=2')
...
def load_route_master(json,source,con):
    ...
    errors=0
    org_id=2
    cursor.execute('SELECT route_path_id,route_description FROM alpl_route_master where org_id=2')
    result = cursor.fetchall()
    pathList = [i[0] for i in result]
    
    if ("MINING" in source):
        columMap = {'route_path_id': 'RoutePathID', 'alpl_route_id': 'RouteID', 'from_location': 'Unit','unit_id':'UnitCode',
...
def main(file_name):
    ...
    try:
        ...
        print(source)
        func = None
        if 'MINING_ROUTE_MASTER' in source:
            func = load_route_master
        elif source=='MINING_ROUTELEG_MASTER':
            pass
        elif source=='MINING_ROUTEKMPL_MASTER':
            func = load_route_kmpl
        elif 'MINING_LEG_MASTER' in source:
            func = load_leg_master
        elif 'MINING_LOCATIONTYPE_MASTER' in source:
            func =  load_location_type_master
        elif 'MINING_LOCATION_MASTER' in source:
            func = load_location_master
        elif source=='MINING_LEG_TIME_MASTER':
            pass
        if func != None:
