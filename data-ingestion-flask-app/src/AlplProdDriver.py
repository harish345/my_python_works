...
def load_alpl_driver_sureties_master(json,source,con):
    ...
    if(len(json)<=150):
        for item in json:
            driver=driverReg_map.get(int(item.get('DriverID')))
            if driver !=None:
                drivers.add(driver)
        if(len(drivers)>1):
...
def load_alpl_recoveries(json,source,con):
    ...
    for i, item in enumerate(json):
        ...
        try:
            ...
            con.rollback()
            try:
                Util.insert_error_record('alpl_recoveries', source, item, q, str(e),filename,ref_id)
            except Exception as exp:
                logger.error(str(exp))
            else:
                con.commit()
...
def main(file_name):  # confirms that the code is under main function
    ...
    try:
        global filename
        filename = file_name
        json_obj = Util.getJsonFile(Util.driver_bucket,file_name)
        ...
        src = json_obj[source]
        func =None
        if(src=="DRIVER_MASTER"):
            func = load_alpl_driver_list_master
        elif(src=="DRIVER_VEHICLEMAPPING_TRANS"):
            pass
            if len(jsonData)>=5000:
                Util.truncate_table("alpl_driver_vehicle_mapping")
            func=load_alpl_driver_vehicle_mapping
            stored_proc='update_unassigned'
        elif(src=="DRIVER_ONBOARD_TRANS"):
            pass
            if len(jsonData)>5000:
                pass
                Util.truncate_table("alpl_driver_transactional")
            func=load_alpl_driver_onboarding
        elif(src=="DRIVER_SURITIES_TRANS"):
            pass
            if len(jsonData)>1000:
                pass
                Util.truncate_table("alpl_driver_sureties_master")
            func = load_alpl_driver_sureties_master
        elif(src=="DRIVER_DUTYPERIOD_TRANS"):
            pass
            func = load_alpl_driver_duty_period
        elif(src=="DRIVER_REST_LEAVES_TRANS"):
            pass
            func = load_alpl_driver_leaves_and_rest
        elif(src=="DRIVER_RESUMING_TRANS"):
            pass
            func = load_alpl_driver_timesheet
            stored_proc='update_timesheet'
        elif(src=="DRIVER_ONHOLD_TRANS"):
            pass
            if len(jsonData)>1000:
                Util.truncate_table("alpl_driver_onhold_txn")
            func = load_driver_onhold_txn
        elif(src=="DRIVER_HEALTH_MEDI_TRANS"):
            pass
            func = load_driver_medi_claim
        elif(src=="DRIVER_INSTALLMENTS_TRANS"):
            func = driver_installments
        elif("DRIVER_RECOVERY" in src):
            func = load_alpl_recoveries
        if func != None:
        ...
        Util.move_blob(Util.driver_bucket, file_name, dest_bucket, file_name)
        ...
        Util.move_blob(Util.driver_bucket, file_name, Util.error_bucket, file_name)
