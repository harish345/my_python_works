...
def validate_string(val, val1):
    ...
    if val1 == 'trip_creation_time' or val1 == 'trip_close_time' or val1 == 'order_created_date' or val1 == 'order_created_date' or val1=='arrival_time' or val1=='departure_time' or val1=='created_date_time' or val1=='arrival_time' or val1=='departure_time' or val1=='created_date_time':
        if val != None and val != "":
            ...
            return 'NULL'
    elif val1 == 'bll_date' or val1 == 'dfd_created_date' or val1 == 'effective_from' or val1 == 'effective_to' or val1 == 'order_created_date' or val1 == 'order_completion_date':
        if (val == '0000-00-00'):
            return 'NULL'
        if val != None and val != "":
            return "'" + datetime.strptime(val, '%Y-%m-%d %I:%M:%S %p').strftime('%Y-%m-%d %H:%M:%S') + "'"
...
def get_route_ids(cursor):
    cursor.execute('SELECT CONCAT(from_location,to_location),route_id FROM alpl_route_master where org_id=3')
    result = cursor.fetchall()
...
def load_trip_trans(json,source,con):
    ...
    cursor.execute('select alpl_trip_id from alpl_trip_data where org_id=3 and is_child_trip=False')
    result = cursor.fetchall()
    ...
    cursor.execute('select lm.alpl_reference_id, u.unit_id from alpl_unit_master u, alpl_location_master lm where u.alpl_unit_code::varchar=lm.location_code and u.org_id='+str(org_id)+' and lm.org_id='+str(org_id))
    result = cursor.fetchall()
...
def load_trip_dc(json,source,con):
    ...
    cursor.execute('SELECT dc_no FROM alpl_dc_details where unit_id in (select unit_id from alpl_unit_master where org_id=3)')
    result = cursor.fetchall()
    dc_list = [i[0] for i in result]
    
    cursor.execute('select alpl_trip_id,trip_id from alpl_trip_data where org_id=3 and is_child_trip=False')
    result = cursor.fetchall()
    ...
    for i, item in enumerate(json):
        ...
        try:
            if(item.get('DCNo') in dc_list):
                ...
                if qty=='' or qty==None:
                    qty='null'
                order = str(item.get('OrderID','NULL'))    
                updateQuery = updateQuery +'trip_id = '+str(value)+' ,alpl_driver_id = '+str(driver)+' ,dc_date='+validate_string(item.get('dcdate',None), 'dc_date')+' ,order_id = '+"'"+str(order)+"'"+ ', quantity = '+ str(qty)+' ,dc_type_id='+str(item.get('DcTypeID','NULL'))+' ,last_updated_time=now() where dc_no = '+ "'"+str(item.get('DCNo'))+"'"
                ...
                for table_column, json_column in columMap.items():
                    if i == 0:
                        ...
                        if (table_column == 'order_id'):
                            value = str(item.get(json_column,'NULL'))  
...
def load_trip_route_txn(json,source,con):
    ...
    errors = 0
    q=''
    cursor.execute('SELECT distinct alpl_trip_id FROM alpl_trip_data where org_id=3 and is_child_trip=false')
    result = cursor.fetchall()
    tripList = [i[0] for i in result]
    
    cursor.execute('''SELECT distinct alpl_trip_id || '-' || route_path_id FROM alpl_trip_route_trans
     where extract(year from created_time)=extract(year from current_date) and alpl_trip_id in (select alpl_trip_id from alpl_trip_data where org_id=3)''')
    result = cursor.fetchall()
    trlist = [i[0] for i in result]
    
    cursor.execute('SELECT route_path_id,route_id FROM alpl_route_master where org_id=3')
    result = cursor.fetchall()
    route_path_map=dict(result)
    for i, item in enumerate(json):
        trip_id = item.get("TripID", None)
        route_path_id =item.get("RoutePathID", None)
        insert_query = 'insert into alpl_trip_route_trans(alpl_trip_id,route_path_id,created_time,last_updated_time) values('
        try:
            if(str(trip_id)+'-'+str(route_path_id) in trlist):
                ...
                insert_query = insert_query + str(trip_id)+','+str(route_path_id)+',now(),now() )'
                q=insert_query
...
def load_trip_route_leg_trans(json,source,con):
    ...
    mining=False
    errors = 0
    cursor.execute('SELECT alpl_trip_id FROM alpl_trip_data where unit_id in (select unit_id from alpl_unit_master where org_id=3) and is_child_trip=False and is_completed=0')
    result = cursor.fetchall()
...
def main(file_name):  # confirms that the code is under main function
    ...
    try:
        ...
        if source== 'RMC_TRIP_TRANS':
            func = load_trip_trans
        elif source == 'RMC_TRIP_ROUTE_TRANS':
            func = load_trip_route_txn
            #stored_proc = "update_trip_route"
        elif source== 'RMC_TRIP_DC_TRANS':
            func=load_trip_dc
        elif source== 'RMC_TRIP_ROUTELEGS_TRANS':
            pass
            func=load_trip_route_leg_trans
            stored_proc='update_leg_status_rmc'
