...
def get_location_type(cursor):
    global loct_map
    cursor.execute("SELECT alpl_reference_id,alpl_location_type_id FROM alpl_location_type_master where org_id=3")
    res_lt  = cursor.fetchall()
    loct_map=dict(res_lt)

def get_locations(cursor):
    global loc_map
    cursor.execute("SELECT alpl_reference_id,alpl_location_master_id FROM alpl_location_master where org_id=3")
...
def load_route_master(json,source,con):
    ...
    errors=0
    org_id=3
    cursor.execute('SELECT route_path_id,route_description FROM alpl_route_master where org_id=3')
...
def main(file_name):
    ...
    try:
        ...
        print(source)
        func = None
        if 'RMC_ROUTE_MASTER' in source:
            func = load_route_master
        elif source=='RMC_ROUTELEG_MASTER':
            pass
        elif source=='RMC_ROUTEKMPL_MASTER':
            func = load_route_kmpl
        elif 'RMC_LEG_MASTER' in source:
            func = load_leg_master
        elif 'RMC_LOCATIONTYPE_MASTER' in source:
            func =  load_location_type_master
        elif 'RMC_LOCATION_MASTER' in source:
            func = load_location_master
        elif source=='RMC_LEGTIME_MASTER':
            pass
        if func != None:
