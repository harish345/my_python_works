...
cursor = con.cursor()
cursor.execute('SELECT alpl_leg_id from alpl_dev_redesign.alpl_leg_master')
result = cursor.fetchall()
legList = [i[0] for i in result]

cursor.execute('SELECT alpl_location_master_id from alpl_dev_redesign.alpl_location_master')
result=cursor.fetchall()
locationIdList=[i[0] for i in result]

cursor.execute('SELECT alpl_location_type_id from alpl_dev_redesign.alpl_location_type_master')
result=cursor.fetchall()
locationTypeIdList=[i[0] for i in result]

cursor.execute('SELECT alpl_supplier_id from alpl_dev_redesign.alpl_vendor_master')
result=cursor.fetchall()
...
cursor.execute('SELECT alpl_unit_code,unit_id FROM alpl_dev_redesign.alpl_unit_master')
result = cursor.fetchall()
...
cursor.execute('SELECT alpl_state_id,state_master_id FROM alpl_dev_redesign.alpl_state_master')
result = cursor.fetchall()
...
def load_asset_master(bucketname, filename_with_path):
    ...
    for i, item in enumerate(json_obj["DATA"]):
        dataset = 'values ('
        query = 'INSERT INTO alpl_dev_redesign.alpl_asset_master ('
        updateQuery = 'UPDATE alpl_dev_redesign.alpl_asset_master SET '
        i = 0
        flag = 0
        valueID = '0'
        q = None
                ...
                if (flag == 1) or (table_column == 'asset_reg_number' and (
                    ...
                    if (i == 0):
                        ...
                        if (table_column == 'asset_model_id'):
                            assetModelIdQuery = 'SELECT asset_model_id FROM alpl_dev_redesign.alpl_asset_model WHERE alpl_asset_model_id = '
                            if (value == None):
                                value = ""
                            else:
...
def load_asset_master_mining(bucketname, filename_with_path):
    print("Data ingestion starting for asset_master")
    columMap = {
...
def load_leg_master(bucketname, filename_with_path):
    ...
    if re.match("MINING",type):
        ...
        org=1
    listQuery="SELECT alpl_leg_id from alpl_dev_redesign.alpl_leg_master where org_id="+str(org)
    cursor.execute(listQuery)
    result=cursor.fetchall()
    legIdList=[i[0] for i in result]
    for i, item in enumerate(json_obj["data"]):
        dataset = 'values ('
        query = 'INSERT INTO alpl_dev_redesign.alpl_leg_master ('
        updateQuery = 'UPDATE alpl_dev_redesign.alpl_leg_master SET '
        i = 0
        flag = 0
        valueID = '0'
        try:
...
def load_leg_time_master(bucketname, filename_with_path):
    ...
    for i, item in enumerate(json_obj["data"]):
        ...
        dataset = 'values ('
        query = 'INSERT INTO alpl_dev_redesign.alpl_leg_time_master ('
        updateQuery = 'UPDATE alpl_dev_redesign.alpl_leg_time_master SET '
        i = 0
        flag = 0
        valueID = '0'
        id = 0
        ...
        try:
            ...
            assetIdQuery = 'SELECT asset_model_id FROM alpl_dev_redesign.alpl_asset_model WHERE alpl_asset_model_id = '
            ...
            legIdQury = 'SELECT leg_master_id FROM alpl_dev_redesign.alpl_leg_master WHERE alpl_leg_id = '
            ...
            searchQuery = 'SELECT count(*) from alpl_dev_redesign.alpl_leg_time_master where (leg_id = '
...
def asset_model():
    ...
    column_map = {'alpl_asset_model_id': 'VEH_MODEL_ID', 'asset_model_name': 'VEH_NAME','asset_manufacturer':'MANUFACTURER'}

    cursor.execute('SELECT alpl_asset_model_id from alpl_dev_redesign.alpl_asset_model')
    result = cursor.fetchall()
    ...
    for i, item in enumerate(json_obj["DATA"]):
        dataset = 'values ('
        insertQuery = 'INSERT INTO alpl_dev_redesign.alpl_asset_model ('
        updateQuery = 'UPDATE alpl_dev_redesign.alpl_asset_model SET '
        i = 0
        flag = 0
        valueID = '0'
        value = None
...
def load_location_master(bucketname, filename_with_path):
    ...
    listQuery = "SELECT alpl_reference_id from alpl_dev_redesign.alpl_location_master where org_id=" + str(org)
    cursor.execute(listQuery)
    result = cursor.fetchall()
    locationIdList = [i[0] for i in result]
    for i, item in enumerate(json_obj["data"]):
        dataset='values ('
        insertQuery='INSERT INTO alpl_dev_redesign.alpl_location_master ('
        updateQuery='UPDATE alpl_dev_redesign.alpl_location_master SET '
        j=0
        flag=0
        value='0'
        try:
...
def load_location_type_master(bucketname, filename_with_path):
    ...
    if re.match("MINING",type):
        ...
        org=1
    print(org)
    listQuery="SELECT alpl_reference_id from alpl_dev_redesign.alpl_location_type_master where org_id="+str(org)
    cursor.execute(listQuery)
    result=cursor.fetchall()
    locationTypeIdList=[i[0] for i in result]
    for i, item in enumerate(json_obj["data"]):
        dataset=' values ('
        insertQuery='INSERT INTO alpl_dev_redesign.alpl_location_type_master ('
        updateQuery='UPDATE alpl_dev_redesign.alpl_location_type_master SET '
        i=0
        flag=0
        value='0'
        try:
...
def load_trip_route_leg_trans(bucketname, filename_with_path):
    ...
    cursor.execute('select CONCAT(alpl_trip_id,route_path_id,leg_id) from  alpl_dev_redesign.alpl_trip_route_legs_transaction ')
    res=cursor.fetchall()
    ...
    if("MINING" in json_obj["Source"] ):
        legQ='select leg_master_id from alpl_dev_redesign.alpl_leg_master where org_id=2 and alpl_leg_id='
        mining=True
    else:
        legQ='select leg_master_id from alpl_dev_redesign.alpl_leg_master where org_id=1 and alpl_leg_id='
    for i, item in enumerate(json_obj["data"]):

        dataset='values ('
        query='INSERT INTO alpl_dev_redesign.alpl_trip_route_legs_transaction ('
        updateQuery = 'UPDATE alpl_dev_redesign.alpl_trip_route_legs_transaction SET '
        i = 0
        flag = 0
        valueID = '0'
        tripID = 0
        ...
        try:
            for table_column,json_column in columMap.items():
                ...
                #     arrivalTimeQuery = "select max(arrival_time) from alpl_dev_redesign.alpl_trip_route_legs_transaction where alpl_trip_id = "+ str(tripID) + ' and route_path_id=' + str(routhPathId)
...
def load_parts_master(bucketname, filename_with_path):
    ...
    cursor.execute('SELECT unit_id,id FROM alpl_dev_redesign.alpl_unit_dept_mapping')
    result = cursor.fetchall()
    ...
    cursor.execute('SELECT location,parts_location_master_id FROM alpl_dev_redesign.alpl_parts_location_master')
    result = cursor.fetchall()
    ...
    cursor.execute('select concat(batch,alpl_part_id,store_id,unit_dept_id) from  alpl_dev_redesign.alpl_parts_master ')
    res = cursor.fetchall()
    ...
    columMap = {'alpl_part_id': 'PART_ID',
                ...
                'part_sub_category': 'PART_SUB_CATG',
    ...
    for i, item in enumerate(json_obj["DATA"]):
        ...
        dataset = ' values ('
        insertQuery = 'INSERT INTO alpl_dev_redesign.alpl_parts_master ('
        updateQuery = 'UPDATE alpl_dev_redesign.alpl_parts_master SET '
        i = 0
        flag = 0
        valueID = '0'
        batch = None
...
def load_parts_trans(bucketname, filename_with_path):
    print("Data ingestion starting for alpl_parts_trans")
    #json_obj = getJsonFile(bucketname, filename_with_path)
    partsLists = []
    cursor = con.cursor()
    cursor.execute('select alpl_part_trans_id from  alpl_dev_redesign.alpl_parts_trans ')
    res=cursor.fetchall()
    ...
    for i, item in enumerate(json_obj["DATA"]):
        #print('_____________________________________________________')
        dataset = ' values ('
        insertQuery = 'INSERT INTO alpl_dev_redesign.alpl_parts_trans ('
        updateQuery = 'UPDATE alpl_dev_redesign.alpl_parts_trans SET '
        i = 0
        flag = 0
        valueID = '0'
        try:
...
def status_update(bucketname, filename_with_path):
    print("Data ingestion starting for alpl_trip_route_legs_transaction")
    #json_obj = getJsonFile(bucketname, filename_with_path)
    tripRouteLegLists = []
    cursor = con.cursor()
...
def load_shift_schedule(bucketname, filename_with_path):
    print("Data ingestion starting for alpl_shift_schedule")
    #json_obj = getJsonFile(bucketname, filename_with_path)
    partsLists = []
    cursor = con.cursor()
    # cursor.execute('select alpl_part_id from  alpl_dev_redesign.alpl_parts_master ')
    # res=cursor.fetchall()
    # partsLists = [i[0] for i in res]
    ctu = 0
    cti = 0
    ...
    for i, item in enumerate(json_obj["DATA"]):
        try:
            dataset = 'values ('
            query = 'INSERT INTO alpl_dev_redesign.alpl_shift_schedule ('
            i = 0
            for table_column, json_column in columMap.items():
                if i == 0:
                    query = query + table_column
...
def salary(bucketname, filename_with_path):
    ...
    uct = 0
    ict = 0
    empIdQuery = 'SELECT emp_id FROM alpl_dev_redesign.alpl_hr_salary_maintenance'
    ...
    for i, item in enumerate(json_obj["DATA"]):
        ...
        dataset = 'values ('
        query = 'INSERT INTO alpl_dev_redesign.alpl_hr_salary_maintenance ('
        updateQuery = 'UPDATE alpl_dev_redesign.alpl_hr_salary_maintenance SET '
        i = 0
        flag = 0
        valueID = None
...
def godown_stock(bucketname, filename_with_path):
    ...
    uct = 0
    ict = 0
    empIdQuery = 'select concat(rcl_cust_id,location_id,grade) from alpl_dev_redesign.alpl_godown_stock_trans'
    cursor.execute(empIdQuery)
    result = cursor.fetchall()
    ...
    for i, item in enumerate(json_obj["data"]):
        ...
        dataset = ' values ('
        query = 'INSERT INTO alpl_dev_redesign.alpl_godown_stock_trans ('
        updateQuery = 'UPDATE alpl_dev_redesign.alpl_godown_stock_trans SET '
        i = 0
        flag = 0
        valueID = None
        custId = None
...
def non_working_drivers(bucketname, filename_with_path):
    ...
    uct = 0
    ict = 0
    driverIdQuery = 'select driver_id from alpl_dev_redesign.alpl_non_working_drivers'
    ...
    for i, item in enumerate(json_obj["DATA"]):
        ...
        dataset = ' values ('
        query = 'INSERT INTO alpl_dev_redesign.alpl_non_working_drivers ('
        updateQuery = 'UPDATE alpl_dev_redesign.alpl_non_working_drivers SET '
        i = 0
        flag = 0
        valueID = None
        q = None
...
def state_master(bucketname, filename_with_path):
    print("Data ingestion starting for alpl_non_working_drivers")
    #json_obj = getJsonFile(bucketname, filename_with_path)
    cursor = con.cursor()
    errors = 0
    ...
    uct = 0
    ict = 0
    stateIdQuery = 'select alpl_state_id from alpl_dev_redesign.alpl_state_master'
    ...
    for i, item in enumerate(json_obj["data"]):
        ...
        dataset = ' values ('
        query = 'INSERT INTO alpl_dev_redesign.alpl_state_master ('
        updateQuery = 'UPDATE alpl_dev_redesign.alpl_state_master SET '
        i = 0
        flag = 0
        valueID = None
        q = None
...
def load_monthly_target_master(bucketname, filename_with_path):
    ...
    for i, item in enumerate(json_obj["data"]):
        try:
            dataset = 'values ('
            query = 'INSERT INTO alpl_dev_redesign.alpl_monthly_target_master ('
            i = 0
            for table_column, json_column in columMap.items():
                if i == 0:
                    query = query + table_column
...
def district_master(bucketname, filename_with_path):
    ...
    if ("MINING" in json_obj["Source"]):
        src = 'mining'
    districtIdQuery = 'select alpl_district_id from alpl_dev_redesign.alpl_district_master'
    ...
    for i, item in enumerate(json_obj["data"]):
        ...
        dataset = ' values ('
        query = 'INSERT INTO alpl_dev_redesign.alpl_district_master ('
        updateQuery = 'UPDATE alpl_dev_redesign.alpl_district_master SET '
        i = 0
        flag = 0
        valueID = None
        q = None
...
def supplier_master(bucketname, filename_with_path):
    ...
    uct = 0
    ict = 0
    districtIdQuery = 'select alpl_supplier_id from alpl_dev_redesign.alpl_supplier_master'
    cursor.execute(districtIdQuery)
    result = cursor.fetchall()
    ...
    for i, item in enumerate(json_obj["DATA"]):
        ...
        dataset = ' values ('
        query = 'INSERT INTO alpl_dev_redesign.alpl_supplier_master ('
        updateQuery = 'UPDATE alpl_dev_redesign.alpl_supplier_master SET '
        i = 0
        flag = 0
        valueID = None
        q = None
...
def external_indent(bucketname, filename_with_path):
    ...
    cursor.execute('SELECT alpl_asset_model_id,asset_model_id FROM alpl_dev_redesign.alpl_asset_model')
    result = cursor.fetchall()
    ...
    for i, item in enumerate(json_obj["DATA"]):
        try:
            dataset = 'values ('
            query = 'INSERT INTO alpl_dev_redesign.alpl_diesel_indent_external ('
            i = 0
            for table_column, json_column in columMap.items():
                if i == 0:
                    query = query + table_column
...
def internal_indent(bucketname, filename_with_path):
